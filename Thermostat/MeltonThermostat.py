    ...
    def updateLights(self):
        ...
        if state == 'off':
            ...
            if temp < self.setPoint:
                # below setpoint, HEAT needs to run, red LED fades
                redLight.pulse(fade_in_time=1, fade_out_time=1, n=None, background=True)
                blueLight.off()
            else:
                # at/above setpoint, HEAT satisfied, red solid
                redLight.on()
                blueLight.off()
        elif state == 'cool':
            if temp > self.setPoint:
                # above setpoint, COOL needs to run, blue LED fades
                blueLight.pulse(fade_in_time=1, fade_out_time=1, n=None, background=True)
                redLight.off()
            else:
                # at/below setpoint, COOL satisfied, blue solid
                blueLight.on()
                redLight.off()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                state_name = self.current_state.id.upper()  # OFF/HEAT/COOL
                lcd_line_2 = f"{state_name} SP:{self.setPoint}F".ljust(16)[:16]
                altCounter = altCounter + 1
                if(altCounter >= 11):
...
##
greenButton = Button(24, pull_up=True, bounce_time=0.2)
